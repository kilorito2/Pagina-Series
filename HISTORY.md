...
Bitácora fase por fase de cómo se construyó **AnimeVerse**: decisiones
tomadas, problemas encontrados y cómo se probó cada parte. Para una
descripción general del proyecto ver [`README.md`](./README.md); para el
estado ejecutivo del MVP (qué es real, qué falta, limitaciones conocidas)
ver [`MVP.md`](./MVP.md).

Generado a partir de `prompt-plataforma-streaming.md`, construido por
fases según el **ORDEN DE CONSTRUCCIÓN** del prompt, más una fase extra de
pulido.

## Variables asumidas

El bloque `VARIABLES` del prompt original no venía completado. El nombre,
dominio y origen del video quedaron en lo que traía el ejemplo; la paleta
de colores sí se definió a propósito, "Blue Cinema" (todo vive en
